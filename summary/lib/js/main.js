/**
 * Freemind Viewer V0.3.0
 * Hochschule Furtwangen University
 * Authors: 
 * 	Konstantin Röhm, student research project, Summer 2014
 * 	Jirka Dell'Oro-Friedl, project initiator and maintainer
 */
...
$(document).ready(function() {
	...
	var params = getUrlSearchJson();
	if (params["map"] == undefined) {
		params["path"] = ".";
		params["map"] = "index.mm";
	}
	var url = params["path"] + "/" + params["map"];
	newData(url);
...
/*
 * initEventHandler create event handlers
 * 
 * return void
 */
...
/*
 * startView create Mindmap View and fade Upload View
 * 
 * return void
 */
function startView() {
...
/*
 * switchView switch view between list view and mindmap view
 * 
 * return void
 */
function switchView() {
...
/*
 * createList create list view and change GUI
 * 
 * return void
 */
function createList() {
...
/*
 * newData load new Mindmap Data as XML
 * 
 * @param url filelink return void
 */
...
/*
 * getScale get current scale from label
 * 
 * return float
 */
function getScale() {
...
/*
 * setScaleLabel set scale label text
 * 
 * return void
 */
...
/*
 * setScale scale main layer
 * 
 * @param scale new scale value return void
 */
...
/*
 * newMap create new mindmap
 * 
 * @param data xml data return void
 */
...
/*
 * initializeArrowlinks draw graphic links in the mindmap if the two nodes are
 * visible
 * 
 * return void
 */
...
/*
 * drawArrow draw arrow for the graphic link ending
 * 
 * @param fromx startX value @param fromy startY value @param tox endX value
 * @param toy endY value return Kinetic.Line
 */
...
/*
 * String.prototype.getId extend String Object with the function getId
 * 
 * return String
 */
...
/*
 * parseAttributes analyse and parse the xml node attributes
 * 
 * @param node XML node return Object
 */
	...
	if (style.text == undefined) {
		if ($(node).children().eq(0).prop("nodeName") == "richcontent") {
			...
			if (html.match(/<img/i) !== null) {
				...
				style.link = link;
				style.text = '(click to see picture)';
			} else {
...
/*
 * createListView create list view recursive
 * 
 * @param data XML node collection @param string current HTML string for the
 * list return String
 */
...
function parseLink(link) {
	...
	if (link.match(/^#ID/i) !== null) {
		...
		//var targetLink = location.origin + location.pathname;
		/*
		 * if (location.pathname.match(/index.php/i) !== null) { targetLink +=
		 * location.pathname.replace("index.php", ""); } else { targetLink +=
		 * location.pathname; }
		 */
		var params = getUrlSearchJson(); 
		console.log(params["path"]);
		if ( params["path"] != undefined) 
			link = params["path"] + "/" + link;
		else
...
/*
 * iterate create canvas Mindmap recursive; only the nodes who aren't folded
 * 
 * @param arg.preNode parent Kinetic.Group for the new nodes @param arg.data XML
 * node collection @param arg.oneChild determines if only one childrow is
 * created return void
 */
...
/*
 * String.prototype.capitalize helper function to uppercase the first letter of
 * a string
 * 
 * return String
 */
...
/*
 * initializeConnections draw the quadratic curves between the nodes
 * 
 * @param direction direction of the changed mindmap return void
 */
...
/*
 * composeControlPoints calculate the quadratic curve points between the nodes
 * 
 * @param side direction of the new curve @param ancestor parent node @param
 * node current node return Object
 */
...
/*
 * openLink click event handler for the node links
 * 
 * @param event click event object return void
 */
function openLink(event) {
	...
	/*else if (link.match(/http/i) !== null) {
		...
		link = parseLink(link);
		console.log("Try to open: " + link);
		window.open(link);
	}		
}

/*
 * hover extends the Kinetic.Util with a hover function
 * 
 * @param mouseenter mouseEnter Callback @param mouseout mouseOut Callback
 * return self
 */
...
/*
 * root draw root node in the canvas
 * 
 * @param arg.x x-Position @param arg.y y-Position @param arg.text root text
...
/*
 * nodeClick draw root node in the canvas
 * 
 * @param event click event object return void
 */
...
Kinetic.Util.addMethods(Kinetic.Group, {
	/*
	 * toggleHint create and toggle the children circle hint for a node
	 * 
	 * return void
	 */
	toggleHint : function() {
	...
	/*
	 * firstText get the first text in this.children
	 * 
	 * return Kinetic.Text
	 */
	firstText : function() {
	...
	/*
	 * addBranch draw new node with positions from parent
	 * 
	 * @param arg.style several style attributes (text,fontsize etc.) return
	 * Kinetic.Group
	 */
	addBranch : function(arg) {
		...
		if (test.getWidth() > 800) {// Von 400 auf 800 geaendert selbe Groeße
									// wie in FreeMind Vorlage Marko F.
			text.setWidth(800);
	...
	/*
	 * getMaxY calculate the highest vertical position in a node and its
	 * children
	 * 
	 * @param pos previous highest position return int
	 */
	...
	/*
	 * getMinY calculate the lowest vertical position in a node and its children
	 * 
	 * @param pos previous lowest position return int
	 */
	...
	/*
	 * getNextYPos calculate the next vertical position from a previous node
	 * 
	 * return int
	 */
	...
	/*
	 * center center all node in relation to its parent
	 * 
	 * @param recursive determines if the centering goes recursive through the
	 * tree return void
	 */
...
function getUrlSearchJson() {
	try {
		var j = decodeURI(location.search);
		j=j.substring(1).split('&').join('","').split('=').join('":"');
		return JSON.parse('{"' + j + '"}');
	}
	catch(_e) {
		log("Error in URL-Parameters: " +_e)
		return JSON.parse("{}");
	}
}
